...
def embed(frame, secret_msg):
    n_bytes = frame.size // 8
    if len(secret_msg) > n_bytes:
        raise ValueError("Error: Insufficient bytes, need a larger image or less data!")
    
    secret_msg += "*^*^*"
    data_index = 0
    binary_secret_msg = msgtobinary(secret_msg)
...
def encode_vid_data():
    fr = input("Enter the video file (with extension): ")
    cap = cv2.VideoCapture(fr)
    vidcap = cv2.VideoCapture(fr)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    frame_width = int(vidcap.get(3))
    frame_height = int(vidcap.get(4))

    size = (frame_width, frame_height)
    out = cv2.VideoWriter('stego_vid1.mp4', fourcc, 25.0, size)
    max_frame = 0
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
    ...
    while(vidcap.isOpened()):
        ...
        if ret == False:
            break
        if frame_number == n:
            secret_msg = input("Enter the secret message to embed: ")
            key = input("Enter the key: ")
            encrypted_msg = encryption(secret_msg, key)
            print("The encrypted data is: ", encrypted_msg)
            frame = embed(frame, encrypted_msg)
        out.write(frame)
    
    vidcap.release()
    out.release()
    print("\nEncoded the data successfully in the video file: stego_vid1.mp4")
...
def extract(frame):
    ...
    for byte in total_bytes:
        decoded_data += chr(int(byte, 2))
        if decoded_data[-5:] == "*^*^*":
            final_decoded_msg = decoded_data[:-5]
            final_decoded_msg = decryption(final_decoded_msg)
            print("\nThe Encoded data hidden in the Video was:\n", final_decoded_msg)
            return final_decoded_msg
    
    print("Error: Termination sequence not found in the extracted data.")
    return None
...
def decode_vid_data():
    ...
    ret, frame = cap.read()
    if not ret:
        print("Error: Could not read the specified frame.")
        return
